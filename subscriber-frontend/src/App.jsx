...
import WaitingForAssignment from "./components/WaitingForAssignment";
import { getDeviceId } from "./utils/deviceId";
import "./App.css";

const API_BASE = import.meta.env.VITE_API_URL || "https://e-dispy.onrender.com";
const API = `${API_BASE}/api`;
const IDENTIFY_POLL_SECONDS = 15;
...
export default function App() {
  const [screen, setScreen] = useState("identifying"); // identifying | waitingAssignment | login | classSelect | display | noticeboard
  const [authData, setAuthData] = useState(null);
  const [selectedClass, setSelectedClass] = useState(null);
  const [selectedBoard, setSelectedBoard] = useState(null);
  const [deviceId, setDeviceId] = useState(null);
  const [deviceToken, setDeviceToken] = useState(null);
  const [retryCountdown, setRetryCountdown] = useState(IDENTIFY_POLL_SECONDS);
  ...
  const identify = async () => {
    ...
    try {
      const res = await fetch(`${API}/devices/identify`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ device_uid: id }),
      });
      const data = await res.json();
      ...
      if (data.registered && data.device_mode === "board" && data.board) {
        ...
        // Brand-new device, or registered but not yet assigned by an admin.
        setScreen("waitingAssignment");
      }
    } catch {
      // Backend unreachable — keep showing the waiting screen and retry.
      setScreen("waitingAssignment");
    }
  };

  // Identify this device once on first load.
  useEffect(() => {
    identify();
  }, []);

  // While waiting for an admin to assign this device, keep polling.
  useEffect(() => {
    if (screen !== "waitingAssignment") return;
    setRetryCountdown(IDENTIFY_POLL_SECONDS);
    const tick = setInterval(() => {
      setRetryCountdown((prev) => {
        if (prev <= 1) {
          identify();
          return IDENTIFY_POLL_SECONDS;
        }
        return prev - 1;
      });
    }, 1000);
  ...
  return (
    <div>
      {screen === "identifying" && (
        <div style={identifyStyles.container}>
          <div style={identifyStyles.spinner} />
        </div>
      )}

      {screen === "waitingAssignment" && (
        <WaitingForAssignment deviceId={deviceId} secondsUntilRetry={retryCountdown} />
      )}
      ...
      {screen === "classSelect" && authData && (
        <ClassSelectScreen
          ...
          onClassSelected={(cls) => {
            ...
            setDeviceToken(authData.token); // manual override still uses the logged-in user's token
            setScreen("display");
          }}
      ...
      {screen === "display" && selectedClass && (
        <Display
          ...
          deviceId={deviceId}
          onExitKiosk={() => {
            // Manual exit always falls back to the login flow (e.g. for an
            // admin manually overriding an auto-assigned display).
            setScreen("login");
